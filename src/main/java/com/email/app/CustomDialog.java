   ...
   /* public void showFxmlDialog(String fxml_file, String title) {
    ...
    public void showFxmlFullDialog(String fxml_file, String title) {
        ...
        try {
            ...
            Parent root = FXMLLoader.load(Objects.requireNonNull(CustomDialog.class.getResource(fxml_file)));
            stage3 = new Stage();
          //  stage3.getIcons().add((new ImageLoader().load("EMAIL APPLICATION")));
            stage3.setTitle(title);

            Stage activeStage = null;
            ArrayList<Stage> activeStages = ViewFactory.activeStages;

            for (Stage value : activeStages) {
                activeStage = value;
            }
            stage3.initOwner(activeStage);
            stage3.initModality(Modality.APPLICATION_MODAL);
            Scene scene = new Scene(root);
            scene.getStylesheets().add(Objects.requireNonNull(getClass().getResource("css/table.css")).toExternalForm());
